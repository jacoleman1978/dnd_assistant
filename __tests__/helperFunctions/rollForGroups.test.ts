...
import {rollD20sForGroup, groupRollStats, groupCritical, getGroupRollResults} from "../../src/helperFunctions/rollForGroups";
...
describe("rollD20sForGroup", () => {
    test("should return an object where the total number of rolls is equal to the number of rolls requested, 10", () => {
        ...
        const totalRolls = Object.values(result).reduce((acc, rolls) => acc + rolls.length, 0);

        expect(totalRolls).toBe(numberOfRolls);
    });

    test("should return an object where the total number of rolls is equal to the number of rolls requested, 1000", () => {
    ...
    test("should return objects with 'Disadvantage' having the most 1s, 'Advantage' having the least 1s and 'Disadvantage' having the least 20's and 'Advantage' having the most 20s", () => {
        ...
        const disadvantageResult = rollD20sForGroup(numberOfRolls, 'Disadvantage');
        ...
        const normalOnes = nomralResult['1'].length;
        const advantageOnes = advantageResult['1'].length;

        const disadvantageTwenties = disadvantageResult['20'].length;
        const normalTwenties = nomralResult['20'].length;
        const advantageTwenties = advantageResult['20'].length;

        expect(disadvantageOnes).toBeGreaterThan(normalOnes);
        expect(normalOnes).toBeGreaterThan(advantageOnes);

        expect(disadvantageTwenties).toBeLessThan(normalTwenties);
        expect(normalTwenties).toBeLessThan(advantageTwenties);
    });
});
...
describe("groupRollStats", () => {
    ...
    test("should return an object where the number who succeeded and failed is equal to the number of rolls requested, 1000", () => {
        const numberOfRolls = 1000;
        const advantageType = 'Normal';
        const targetDC = 10;
        const modifier = 0;
...
describe("groupCritical", () => {
    ...
    test("should return a Criticals object for magic attacks", () => {
        ...
        const groupCriticals = groupCritical(groupRolls, damageType, charLevel, spellLevel);
    ...
    test("should return a Criticals object with 'hits' attribute populated and the number of Table and Normal critical hits equals the number of groupRolls['20'] for melee attacks", () => {
        ...
        const groupCriticals = groupCritical(groupRolls, damageType, charLevel);
        const numberOfTableCriticals = groupCriticals.hits.whoHasTableCritical.length;
        const numberOfNormalCriticals = groupCriticals.hits.whoHasNormalCritical.length;
        ...
        expect(numberOfTableCriticals + numberOfNormalCriticals).toBe(numberOfTwenties);
    });
    ...
    test("should return a Criticals object with 'misses' attribute populated and the number of Table and Normal critical misses equals the number of groupRolls['20'] for melee attacks", () => {
        ...
        const groupCriticals = groupCritical(groupRolls, damageType, charLevel);
        const numberOfTableCriticals = groupCriticals.misses.whoHasTableCritical.length;
        const numberOfNormalCriticals = groupCriticals.misses.whoHasNormalCritical.length;
        ...
        expect(numberOfTableCriticals + numberOfNormalCriticals).toBe(numberOfOnes);
    });
...
describe("getGroupRollResults", () => {
    ...
    test("should return a GroupRollStats object with all attributes populated for melee attacks", () => {
        ...
        expect(result.whoSucceeded.length + result.whoFailed.length).toBe(groupRollInputs.numberOfRolls);
    });
